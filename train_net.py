...
def train_net(cfg):
    ...
    # Reading dataset
    #training_set一共有3493个sample,形式是：4,...--->4代表的是返回的一共四种，images,bboxes,actions,group_activity,T,C,H,W
    ...
    training_loader=data.DataLoader(training_set,**params)#437，
    #print(len(training_loader))437的意思时一次batch有8个，一共3493个，所以需要473次dataloader
    ...
    if cfg.training_stage==1:
        ...
        #跳到class Basenet_volleyball(nn.Module)
        model=Basenet(cfg)
    ...
    print("这是训练之前，basenet的forward之后前")
    ...
    for epoch in range(start_epoch, start_epoch+cfg.max_epoch):
        print("进入第"+str(epoch)+"个epoch")
        if epoch in cfg.lr_plan:
            adjust_lr(optimizer, cfg.lr_plan[epoch])
            
        # One epoch of forward and backward
        # train()--->train_volleyball()
        '''
        train()--->train_volleyball()
        traing_loader:Dataloader处理过的training_set,training_set的形式是：images, bboxes,  actions, activities
        model:默认使用inception v3
        device:gpu
        optimizer:Adam
        epoch:1,2,3...
        cfg:配置文件
...
def train_volleyball(data_loader, model, device, optimizer, epoch, cfg):
    ...
    epoch_timer=Timer()
    for batch_data in data_loader:#batch_data的大小和batch_size一致
        model.train()
        model.apply(set_bn_eval)

        #batch_data是list，保存了images,bboxes,actions,group_actions 4个.
        #images b x T x c x h x w
        #bboxes b x T x 人数 x 坐标
        #actions b x T x 人数
        #group_actions b x T
        # prepare batch data
        ...
        actions_in=batch_data[2].reshape((batch_size,num_frames,cfg.num_boxes))# 将 actions 转换为 b x T x 人数
        
        activities_in=batch_data[3].reshape((batch_size,num_frames))# 将 群体行为标签 转换为 b x T
        
       
        actions_in=actions_in[:,0,:].reshape((batch_size*cfg.num_boxes,))# 取第一帧动作 转换为 b x 0 x T --> b*T
        
        activities_in=activities_in[:,0].reshape((batch_size,))# 取第一帧群体动作 转换为 b x 0 --> b
     
        

        actions_scores,activities_scores=model((batch_data[0],batch_data[1]))

        #print("我执行完了BaseNet的前向函数了")
        # Predict actions
